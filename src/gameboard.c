...
/*   Updated: 2023/01/18 17:19:26 by cariencaljo   ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	init_images(mlx_t *mlx, t_textures *text, t_gameboard *gb)
{
	...
	imgs->moves_count = mlx_new_image(mlx, gb->text->nums[0]->width * 4, gb->text->nums[0]->height);
	imgs->coll_count = mlx_new_image(mlx, gb->text->nums[0]->width * 4, gb->text->nums[0]->height);
...
{	
	...
	gb->mlx = mlx_init(gb->width + (gb->text->empty->width * 3), \
